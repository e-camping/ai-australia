...
export default async function handler(request, response) {
  ...
  const file_contents = fs.readFileSync(
    path.join(__dirname, "../python", file),
    "utf-8",
  );
  ...
  const write = await sb.exec(["sh", "-c", `cat > /tmp/${file}`], {
    stdin: "pipe",
    stdout: "pipe",
    stderr: "pipe",
  });
  ...
  const p = await sb.exec(["python", `/tmp/${file}`], {
    stdout: "pipe",
    stderr: "pipe",
  });
  let output = await p.stdout.readText();
  ...
  return response.status(200).json({ quote: output });
}
